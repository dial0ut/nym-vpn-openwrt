...
| `12-remove-install` | remove the package (`opkg remove` / `apk del`, prerm's real-removal path) and install it again: `/etc/nym` survives the removal and prerm says so; afterwards the daemon runs under procd with the same account identity and settings (the kill-switch is set to the non-default off first, so a lost settings file shows) |
| `13-force-reinstall` | `opkg install --force-reinstall` (opkg removes first): same account identity and settings afterwards. apk: SKIP (no forced reinstall of a sideloaded file) |
| `14-downgrade` | with a previous artifact configured: downgrade to it (`--force-downgrade` on opkg) and upgrade back to the package under test; the account identity and settings survive both steps. Without a previous artifact: SKIP |
