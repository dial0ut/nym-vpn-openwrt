...
scenario_check() {
    local st
    note "restart window while disconnected: see the watcher"
    st=$(state); echo "$st"
    if printf '%s' "$st" | grep -q 'policy=yes'; then recovered "Blocked policy hooked after the restart"; else not_recovered "$st"; fi
}
