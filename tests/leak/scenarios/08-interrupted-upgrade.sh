...
# Interrupted upgrade on an opkg router: opkg is SIGKILLed together with the
# post-upgrade script the moment that script starts, i.e. after the new
# files are unpacked and before post-upgrade has done its work (opkg writes
# files in place and only marks the status entry at the very end, so the
# script appearing is the usable signal; opkg runs it as
# `sh -c "... /usr/lib/opkg/info/nym-vpn.postinst configure"`, and the [.]
# in the pattern keeps it from matching this session's own command line,
# which carries the pattern text). prerm leaves the old daemon running on
# upgrade, so expect: no leak, the old daemon still up with its policy
# hooked, and a recovery that completes. Observed on 21.02: opkg has not
# touched its status entry at that point (it still reads the OLD version,
...
scenario_pre() {
    if ! rt 'command -v opkg >/dev/null 2>&1'; then
        skip "router has no opkg (apk router: see 18-upgrade-interrupted-apk)"
        return 1
    fi
...
scenario_check() {
    local out st
    echo "-- after interruption:"
    st=$(state); echo "$st"
    printf '%s' "$st" | grep -qE 'policy=yes .*vpnd=[0-9]' || not_recovered "old daemon or its policy gone after the interruption: $st"
    out=$(rt "want=\$(grep ^Version /usr/lib/opkg/info/nym-vpn.control | cut -d' ' -f2)
    ...
    if printf '%s' "$st" | grep -qE 'policy=yes .*vpnd=[0-9]'; then
        if printf '%s\n' "$out" | grep -q 'recovery 2'; then
            recovered "old daemon kept running through the interruption; opkg configure was a no-op (status still old version); re-running the install completed it with the kill-switch armed"
        else
            recovered "via opkg configure"
        fi
    else
        not_recovered "recovery did not complete: $st"
    fi
}
