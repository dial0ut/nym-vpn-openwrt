...
# Rule application failure: iptables-restore is replaced by a script that
# exits 1, then a policy change is requested (IPv6 toggle: it re-applies the
# whole fw3 policy). Expect: fail-closed (the previous rules stay, the daemon
# reports the failure), no leak, and RECOVERY once the real binary is back and
# the setting is toggled back.
scenario_wait=15
scenario_pre() {
...
scenario_inject() {
    ...
    printf '%s\n' "$out"
    [ "$(printf '%s\n' "$out" | grep -c SET_OK)" -gt 0 ] || note "INJECTION FAILED: tunnel set did not succeed"
}
scenario_check() {
    local out
    out=$(rt 'echo "-- while broken:"; nym-vpnc status 2>&1 | head -1 | cut -c1-70
        iptables -w -C output_rule -j NYM_OUTPUT 2>/dev/null && echo "policy hooked (previous rules kept)" || echo "policy NOT hooked"
        echo "emergency chains: $(iptables -w -S | grep -c NYM_EMERGENCY)"
        logread | grep -iE "nym-vpn:|injected failure|SetFirewallPolicy|Failed to (set|apply) firewall|error state" | tail -4 | cut -c1-170
        echo "-- restore binary, toggle back:"; rm -f /usr/sbin/iptables-restore; cp -a /tmp/leak/iptables-restore.bak /usr/sbin/iptables-restore
        nym-vpnc tunnel set --ipv6 off 2>&1 | tail -1; sleep 6
        nym-vpnc status 2>&1 | head -1 | cut -c1-70
    ...
    if [ "$(printf '%s\n' "$out" | grep -c '^recovered: policy hooked')" -gt 0 ]; then
        note "recovered after binary restored + re-apply"
    else
        note "NOT recovered"
    fi
}
