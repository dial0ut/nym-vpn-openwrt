...
| `nym-vpn/components/` | `card` (expandable shell, `group()` sub-headings with an optional title-bar action), `toggle` (switch row: one-clause description, optional `reconnect` tag, `more` text behind the (i) button, save-or-revert), `details` (the shared (i) expander: aria-expanded button + inline panel, remembered per row in localStorage, *Learn more* link into the docs-site LuCI guide), `select`, `modal`, `toast`, `gateway-picker` (country dropdown, server list, saved-selection restore) |
...
shared behaviour through components and flows. The legacy PBR switch in
Split Tunneling greys the kill-switch in Tunnel Settings that way — both read
`store.tunnelSwitches` and listen for its `tunnel-switch` event. `modal` and `toast` are module
