    ...
    C: Serialize,
{
    ...
    // Temp + fsync + rename: a power cut mid-write must never leave a
    // truncated config, which would reset every setting to defaults.
    ...
    // tokio's File reports write errors on the NEXT operation and sync_all()
    // swallows a pending one; without this flush an ENOSPC write "succeeds".
    file.flush()
        .await
        .map_err(|error| ConfigSetupError::WriteFile {
            file: tmp_path.clone(),
    ...
    // Data must reach disk before the rename does, or a power cut still
    // leaves an empty file under the final name.
    file.sync_all()
        .await
        .map_err(|error| ConfigSetupError::WriteFile {
            file: tmp_path.clone(),
