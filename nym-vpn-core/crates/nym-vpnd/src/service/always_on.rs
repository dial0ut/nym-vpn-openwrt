...
impl AlwaysOn {
        ...
        match state {
            ...
            TunnelState::Disconnected => {
                ...
                if self.supervising() && !self.latched {
                    // Reached with the target still Secured on shutdown, at
                    // boot before the first Connect lands, or when a settings
                    // change re-applies the firewall and lifts an Error (the
                    // Connect for that is already on its way). Treat it as an
                    // infrastructure error without deepening the streak.
    ...
    pub fn on_config_or_account_changed(&mut self) -> Action {
        if !self.supervising() || !matches!(self.last_seen, Seen::Error | Seen::Disconnected) {
            return Action::Nothing;
        }
...
mod tests {
    ...
    #[test]
    fn config_change_cuts_a_disconnected_retry_short() {
        let mut ao = supervisor();
        let now = Instant::now();
        ao.on_tunnel_state(&error(ErrorStateReason::SetFirewallPolicy), now);
        // A settings change re-applied the policy and lifted the error.
        ao.on_tunnel_state(&TunnelState::Disconnected, now + secs(1));
        assert!(ao.next_deadline().is_some());
        assert_eq!(ao.on_config_or_account_changed(), Action::Connect);
        assert_eq!(ao.next_deadline(), None);
    }
