...
impl TestBench {
    ...
    /// Assert that we are in a given state within 60 seconds. This is needed to avoid
    /// tokio::sleep and yield_now everywhere.
    ///
    /// If after the 60sec delay, the expected state is not reached, the `assert_eq` call will
    /// fail, with a normal failure.
    ///
