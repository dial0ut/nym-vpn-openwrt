#!/bin/bash
# Build APK package for OpenWrt 25.x+ with apk mkpkg (apk-tools 3.x, native
# or via Docker Alpine).
#
...
for f in fw3-include.sh fw4-include.sh fw-boot-guard.sh fw-rules.sh; do
    ...
    cp "$FW_SCRIPTS_DIR/$f" "$DATA_DIR/usr/share/nym-vpn/"
    case "$f" in
        fw?-include.sh) chmod 755 "$DATA_DIR/usr/share/nym-vpn/$f" ;;
        *) chmod 644 "$DATA_DIR/usr/share/nym-vpn/$f" ;;
    esac
done

# apk looks the ECDSA key up by the basename embedded at signing time (the
# private key passed to `apk mkndx --sign-key` in CI), so the public half
# must ship under that same basename. The signify key is opkg-only.
...
# apk runs pre-upgrade/post-upgrade instead of pre-deinstall/post-install on
# upgrades, and never sets opkg's PKG_UPGRADE=1, which guards prerm's
# destructive branch; wrap it.
{
    echo '#!/bin/sh'
