...
### A resolver on your LAN (Pi-hole, AdGuard Home on another box)

A custom DNS server with a private address (10/8, 172.16/12, 192.168/16, link-local, IPv6 ULA)
...
forwards and follow the tunnel rules. If the daemon cannot determine the WAN interface it logs a
warning and falls back to admitting the private resolver through the tunnel only.
