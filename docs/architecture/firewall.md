...
When connected, the daemon routes all output through the tunnel by default — including the **reply** to any inbound connection from the WAN (this routing is unconditional; the kill-switch only governs whether non-tunnel egress is additionally *blocked*). That breaks port-forwarded services: an external client connects to a public port on the router, the service replies, and the reply takes the tunnel instead of the original WAN path. The source IP at the egress no longer matches what the client connected to, and the connection dies.
...
**3. Routing rule — bypass the tunnel.** An `ip rule fwmark 0x14e lookup main` at priority 90 sits **before** the suppress rule (100) and the tunnel fwmark rule (200). Marked replies hit the main routing table — which has the real WAN as default — instead of table 333 (the tunnel default). This rule is installed for the whole tunnel lifetime (not just when exemptions exist), so it also serves as the carve-out path for [split tunnelling](../guide/split-tunneling.md): any packet marked `0x14e` bypasses the tunnel, whether it is an inbound-service reply or a user-defined outbound exclusion.
