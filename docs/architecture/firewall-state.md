...
| `ifaces` | daemon (fw3 apply with tunnel interfaces) | daemon (apply without interfaces, reset); include (stop marker present) | fw3 include (masquerade, LAN forwarding); fw4 include as an optional hint | fw3 state lock | missing = no tunnel: include removes masquerade and LAN forwarding |
...
rules/ifaces/transition files, with disjoint responsibilities: the daemon
...
| `nym_postrouting`, `nym_forward_lan` in `inet fw4` | daemon | daemon; prerm | fw4 include restores from live tunnel detection after every reload | none (nft transactions) |
...
| `NYM_FORWARD_LAN`, `NYM_POSTROUTING` (fw3 tunnel plane) | daemon | daemon (no interfaces, reset); include (no `ifaces`, stop marker); prerm (inline fallback only) | include restores from `ifaces` | fw3 state lock |
...
| firewall reload | fw3 leaves foreign chains alone; include reconciles (re-hook, lift stale block) | fw4 rebuilds only its own table; include restores `nym_postrouting`/`nym_forward_lan` |
...
| removal | prerm stops the daemon (as above), runs the include under the lock once more, removes the runtime directory and the UCI include | prerm deletes the tables and the in-fw4 chains |
