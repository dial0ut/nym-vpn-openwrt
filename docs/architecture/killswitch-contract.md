...
|---|---|---|---|---|---|---|---|---|
| Daemon idle (Disconnected) | blocked except daemon bootstrap | blocked (LAN-to-LAN allowed if `allow_lan`) | rejected; root-scoped rate-limited hatch for the daemon only | yes when `enable_ipv6`; fw3 refuses to apply if kernel v6 is up and ip6tables is unusable | LAN: fw3/fw4 INPUT policy; WAN sessions: reply direction | `tunnel_state_machine/mod.rs:apply_killswitch_policy` → `policy.rs:compile` Blocked | connect, or turn the kill-switch off | unit:`blocked_policy_terminates_in_reject`, `blocked_state_blocks_forward_lan_to_wan`, `blocked_dns_exceptions_are_root_scoped`, `no_unscoped_dns_output_while_tunnel_down`; capture (fw3, fw4: LAN client blocked while disconnected); harness `30-killswitch-disconnected` |
| Connecting | as Blocked plus peer endpoints and tunnel interface | into the tunnel only | rejected on the WAN; hatch root-scoped | as above | as above | `states/connecting_state.rs:set_firewall_policy` → Connecting | connection completes or fails to Disconnected/Error | unit:`connecting_policy_terminates_in_reject`, `connecting_dns_hatch_is_root_scoped`; capture (connect attempts observed, no LAN egress) |
| Connected | tunnel and peer endpoints only | into the tunnel only | tunnel resolvers via tunnel only; WAN port 53 rejected | as above | as above | `states/connected_state.rs:set_firewall_policy` → Connected | n/a | unit:`connected_forward_output_have_no_unqualified_established_accept`, `connected_lan_collects_tunnel_iface_and_emits_cve_drop`; capture baseline (fw3: 20 s connected, zero non-WireGuard egress); harness `32-killswitch-connected` |
...
| Offline (no WAN) | Blocked-shaped policy with `allow_lan` | as Blocked | as Blocked | as Blocked | as Blocked | `states/offline_state.rs:set_firewall_policy` | WAN returns → reconnect | **unverified** by capture (watchdog link-flap test observed reconnect only) |
| Exemptions configured | unchanged: only *replies* to marked inbound flows leave via the WAN | unchanged plus marked replies | unchanged | v4 and v6 restores share the `ct mark` scoping | unchanged | `policy.rs:exemption_mangle_rules` (restores scoped to `ct mark 0x14e`), `exemption_filter_accepts`, `bypass_mark_forward_accept` | n/a | unit:`exemption_mangle_restores_meta_mark_after_set`, `exemptions_emit_filter_mark_accepts_before_final_reject`, `unmarked_lan_to_wan_still_rejected_with_bypass_accept`; device (fw3 21.02: connmark fixes reproduced and verified, commits d33383d76, 15c93b7d4, 0e6ff520c); **unverified** by capture that an exempt service cannot originate outbound |
...
| Boot: daemon's own bootstrap under the boot block | daemon API/DoH/NTP are **blocked** until its first policy replaces the block (the block has no uid hatches) | n/a | n/a | n/a | n/a | same | daemon applies Blocked (which lifts the block) before it needs the network for anything but discovery | capture (first daemon egress appears seconds after the block is lifted). **Not protected against a startup deadlock on a non-mainnet network with no cached discovery**: discovery runs before the first policy and fails, the daemon exits, the block stays (issue #15) |
...
- **The include scripts are embedded in the daemon binary** (`include_str!`) and installed at
  daemon start. Changing a script under `scripts/` needs a Rust rebuild and a daemon restart to
  reach `/usr/share/nym-vpn/`.
