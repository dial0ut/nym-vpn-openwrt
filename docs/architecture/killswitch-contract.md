...
| Restart from the web UI or the account hard reset (rpcd `daemon_restart`, `account_reset`) | as daemon restart: the bridge runs `/etc/init.d/nym-vpnd restart` and `reset_account` (an `EXTRA_COMMAND`: stop, wipe `/etc/nym/data`, start), both keep paths | same | same | same | same | `nym-vpnc/src/commands/rpcd.rs:daemon_restart`/`account_reset` → init script `keep_killswitch` (`restart`, `reset_account`). The bridge used to run `stop`, sleep and `start`, so the stop hook opened the firewall for the gap; `daemon_stop` still runs `stop`, the documented escape hatch | new daemon's first apply | harness `11-restart-paths` (table present at every 1 s sample across both calls); **unverified** by capture |
...
| Firewall reload (`/etc/init.d/firewall reload`) | fw4: `inet nym` is not fw4's table, untouched. fw3: reload deletes only fw3-tagged rules and skips `*_rule` chains; our chains and jumps survive, the include reconciles. Both: the `nym` zone (masquerade, MSS clamp, `lan -> nym`) is re-rendered by fw3/fw4 from `/etc/config/firewall`, nothing of the daemon's to restore | same | same | same | same | fw4: table separation; fw3: firewall3 `fw3_flush_rules(reload=true)`, include `main → restore_policy` (idempotent); zone: `uci-defaults/luci-app-nym-vpn` | n/a | capture (fw3: policy hooked at every sample, zero leaks; fw4: earlier session); unit:`include_script_names_the_same_tables`, `include_script_only_ever_deletes_the_boot_table` |
...
- **On fw3 our ACCEPT is final.** NYM_* are jumped from `input_rule`/`forwarding_rule`; an
  ACCEPT there ends the builtin chain and skips the zone rejects. Anything the zones must still
  judge is `Verdict::Return` (`-j RETURN`), which on fw4 means what an accept already did.
...
- **The `nym` zone masquerades but is not an uplink.** `common.rs:wan_zone_devices` takes every
  zone named `wan` or with `masq`; the tunnel zone has `masq` too and is skipped by name
  (`NYM_ZONE`). Rename the zone and the exemption mangle rules would mark tunnel-inbound flows
  for the real WAN.
